...
```bash
gh auth login
gh stack version                    # requires GitHub CLI stack support
git config rerere.enabled true
git config remote.pushDefault origin # needed when a repo has multiple remotes
```
...
# Add work on the current top branch
gh stack add tests

# Move around the current stack
gh stack up [n]
gh stack down [n]
gh stack top
gh stack bottom
...
# API-only linking, without local stack tracking
gh stack link <bottom> <middle> <top>
gh stack link <pr-number> <pr-number> <pr-number>

# Remove only the grouping; branches and PRs survive
gh stack unstack
gh stack unstack --local

# Merge when your installed `gh stack` exposes this command
gh stack merge <pr-or-stack-number> -y --squash
```
...
| `Ctrl+S` | Apply | Apply all staged changes. Run `submit` afterward to reconcile GitHub. |
...
the named demo repository. If no interactive terminal is available, use the
safe fallback:
...
- Use `gh stack submit`, not `gh pr create`, for stacked PR creation.
...
- `sync` never creates missing PRs; use `submit` for that.
...
Use `gh stack view --json` as the machine-readable source of truth. Important
fields include `trunk`, `currentBranch`, `branches`, each branch's `base`,
`needsRebase`, and optional PR state.
...
- Keep local stack: `gh stack unstack`, then `gh stack submit --auto --remote origin`.
