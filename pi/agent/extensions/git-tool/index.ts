...
export type GitToolMode = "git-spice" | "main" | "none";

type StackingGitToolMode = "git-spice";
...
function isAllowedBranchCreate(tokens: string[], _mode: StackingGitToolMode): boolean {
	return (
		tokens[0] === "gs" &&
...
function isAllowedToolInspection(tokens: string[], _mode: StackingGitToolMode): boolean {
	return (
...
function gitToolLabel(_mode: StackingGitToolMode): string {
	return "Git-Spice";
}
...
function defaultReadTrunkBranch(_mode: StackingGitToolMode): string | undefined {
	return commandOutput("gs", ["trunk", "-n"]);
}
